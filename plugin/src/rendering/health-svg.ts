...
export function renderHealthSvg(
  ...
  const detail = truncate(health.detail || "", 28);
...
<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 144 144" role="img" aria-label="Orca health ${escapeXml(label)}">
  ...
  <text x="16" y="28" fill="${palette.muted}" font-family="ui-monospace, SFMono-Regular, Menlo, monospace" font-size="11" letter-spacing="1.2">ORCA</text>
  <text x="128" y="28" text-anchor="end" fill="${palette.muted}" font-family="ui-monospace, SFMono-Regular, Menlo, monospace" font-size="10">${escapeXml(version)}</text>
  <line x1="16" y1="34" x2="128" y2="34" stroke="${palette.line}" stroke-width="1"/>
  ${sweep}
  <circle cx="72" cy="52" r="8" fill="${color}"/>
  <text x="72" y="92" text-anchor="middle" fill="${color}" font-family="ui-monospace, SFMono-Regular, Menlo, monospace" font-size="16" font-weight="700" letter-spacing="1.5">${escapeXml(label)}</text>
  <text x="72" y="110" text-anchor="middle" fill="${palette.ink}" font-family="ui-monospace, SFMono-Regular, Menlo, monospace" font-size="11">${escapeXml(runtime)}</text>
  <text x="72" y="126" text-anchor="middle" fill="${palette.muted}" font-family="ui-monospace, SFMono-Regular, Menlo, monospace" font-size="9">${escapeXml(detail)}</text>
</svg>`;
}
